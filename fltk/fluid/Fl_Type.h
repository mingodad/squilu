...
public:
  ...
  const char *macro_name() const {return macro_name_;}
  void macro_name(const char *);
  const char *macro_name2name();
...
class Fl_Block_Type : public Fl_Type {
  FILE *global_out, *block_out;
  const char* file_name;
  const char* after;
  const char* before;
  char public_;
public:
  Fl_Block_Type():Fl_Type(){
      file_name = NULL;
      after = NULL;
      before = NULL;
  }
  ~Fl_Block_Type(){
      if(file_name) free((void*)file_name);
      if(after) free((void*)after);
      if(before) free((void*)before);
  }
  Fl_Type *make();
  void write_code1();
  void write_code2();
  void open();
  virtual const char *type_name() {return "block";}
  void write_properties();
  void read_property(const char *);
  int is_parent() const {return 1;}
...
#define NUM_EXTRA_CODE 6
#define NUM_EXTRA_CODE_STD 4
...
protected:
  ...
  void write_extra_code_num(int n);
...
#define FL_WIDGET_CLASS_FL_GROUP 0
#define FL_WIDGET_CLASS_FL_WIDGET 1

extern Fl_Menu_Item widget_class_type_menu[];
extern const char *current_inner_structure;
const char *check_inner_struct(char *wname, int size_wname, const char *name);
#define CHECK_INNER_STRUCT(buf, s, aname) \
    s = check_inner_struct(buf, sizeof(buf), aname);

class Fl_Widget_Class_Type : private Fl_Window_Type {
protected:

  //Fl_Menu_Item* subtypes() {return widget_class_type_menu;}
public:
  Fl_Widget_Class_Type() {
    write_public_state = 0;
    wc_relative = 0;
    wc_inner_struct = NULL;
    saved_current_inner_structure = NULL;
  }
  ~Fl_Widget_Class_Type(){
      if(wc_inner_struct) free((void *)wc_inner_struct);
  }
  // state variables for output:
  char write_public_state; // true when public: has been printed
  char wc_relative; // if true, reposition all child widgets in an Fl_Group
  const char *wc_inner_struct; // if not null create a inner struct for members
  const char *saved_current_inner_structure;
