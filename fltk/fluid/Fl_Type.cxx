...
void Fl_Type::macro_name(const char *n) {
  storestring(n,macro_name_);
  macro_name2name();
}

const char *Fl_Type::macro_name2name() {
  if(macro_name_){
    char buf[256];
    snprintf(buf, sizeof(buf), "%s", macro_name_);
    for(char *p=buf; *p; ++p){
        if(*p == ' ') *p = '_';
    }
    name(buf);
...
Fl_Type::~Fl_Type() {
  ...
  if (macro_name_) free((void*)macro_name_);
...
void Fl_Type::read_property(const char *c) {
  if (!strcmp(c,"label"))
    ...
    comment(read_word());
  else if (!strcmp(c,"macro_name"))
    macro_name(read_word());
