...
///////////////////////////////////////////////////////////////////////
Fl_Menu_Item widget_class_type_menu[] = {
  {"Fl_Group",0,0,(void*)FL_WIDGET_CLASS_FL_GROUP},
  {"Fl_Widget",0,0,(void*)FL_WIDGET_CLASS_FL_WIDGET},
  {0}};


Fl_Widget_Class_Type Fl_Widget_Class_type;
Fl_Widget_Class_Type *current_widget_class = 0;
const char *current_inner_structure = 0;
...
void Fl_Widget_Class_Type::write_properties() {
  ...
  if (wc_inner_struct) {write_string("inner_struct"); write_word(wc_inner_struct);};
}

void Fl_Widget_Class_Type::read_property(const char *c) {
  if (!strcmp(c,"position_relative")) {
    wc_relative = 1;
  } else if(!strcmp(c,"inner_struct")) {
    storestring(read_word(),wc_inner_struct);
  } else {
...
void Fl_Widget_Class_Type::write_code1() {
  ...
  static const char *inner_struct_format_str = "  struct st_%s {\n";
  const char *c = subclass();
  if (!c) c = "Fl_Group";

  if(strstr(c, "NO_INHERITANCE")){
    write_h("\nclass %s {\n", name());
    ...
    //if (((Fl_Window*)o)->type() == FL_WIDGET_CLASS_FL_GROUP)
    ...
    //    write_c("  %s *w = this;\n", name());
    write_c("begin();\n");
    if(wc_inner_struct) write_h(inner_struct_format_str, wc_inner_struct);
  }

//  write_c("  %s *o = this;\n", name());
  saved_current_inner_structure = current_inner_structure;
  current_inner_structure = wc_inner_struct;
  write_widget_code();
}

void Fl_Widget_Class_Type::write_code2() {
  if(wc_inner_struct) write_h("  } %s;\n", wc_inner_struct);
  current_inner_structure = saved_current_inner_structure;
  write_extra_code();
