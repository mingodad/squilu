...
const char *check_inner_struct(char *wname, int size_wname, const char *name){
  if(name){
      if(current_inner_structure)
        snprintf(wname, size_wname, "%s.%s", current_inner_structure, name);
      else
        snprintf(wname, size_wname, "%s", name);
  } else wname[0] = '\0';
  return wname;
}
...
void name_cb(Fl_Input* o, void *v) {
  ...
  if (v == LOAD) {
    if (numselected != 1) {
      ...
      const char *wname = current_widget->dirty_name() ?
                current_widget->dirty_name() : current_widget->name();
      if(wname && current_widget->is_class() && current_widget->is_window()){
          //assume Fl_Widget_Class_Type
          Fl_Widget_Class_Type *wct = (Fl_Widget_Class_Type*)current_widget;
          const char *format_str;
          if(wct->wc_inner_struct) format_str = "%s$%s";
          else format_str = "%s";
          snprintf(buf, sizeof(buf), format_str, current_widget->name(), wct->wc_inner_struct);
      } else if(wname) strcpy(buf, wname);
      else buf[0] = '\0';
      o->value(buf);
      o->show();
    ...
    if (numselected == 1) {
      if(current_widget->is_class() && current_widget->is_window()){
          //assume Fl_Widget_Class_Type
          ...
          if(ptr) {
              *ptr++ = '\0';
              if(*ptr) storestring(ptr, wct->wc_inner_struct);
          } else {
              storestring("", wct->wc_inner_struct);
          }
          current_widget->name(buf);
      } else {
          const char *aname = o->value();
          current_widget->dirty_name(aname);
          char *aname_clean = replace(aname[0] == '@' ?
                    (aname[1] == '&' ? (aname+2) : (aname+1)) : aname, "@", "_");
...
void macro_name_cb(Fl_Input* o, void *v) {
  if (v == LOAD) {
      o->value(current_widget->macro_name());
  } else {
      current_widget->macro_name(o->value());
  }
}
...
void Fl_Widget_Type::write_static() {
  ...
  for (int n=0; n < NUM_EXTRA_CODE_STD; n++) {
    if (extra_code(n) && isdeclare(extra_code(n)))
      write_declare("%s", extra_code(n));
  }
  const char *before_all = extra_code(4);
  if(before_all && (*before_all == '#')) write_h("%s\n", before_all);
  ...
  const char *after_all = extra_code(5);
  if(after_all && (*after_all == '#')) write_h("%s\n", after_all);
}
...
void Fl_Widget_Type::write_code1() {
  ...
  const char *wname;
  char buf[256];
  CHECK_INNER_STRUCT(buf, wname, name());

  write_c("%s{ ", indent());
  //extra code after widget creation
  const char *before_all = extra_code(4);
  if(before_all) write_c("\n  %s%s\n  %s", indent(), before_all, indent());

  if (varused) write_c("%s* o = ", t);
  if (name()) write_c(no_new ? (varused ? (is_pointer ? "%s;\n" : "&%s;\n") : "\n") :
                      //"%s = ", no_new ? wname : c);
                      "%s = ", wname);
  if(no_new){
     write_c("%s  Fl_Group::current()->add(%s);\n", indent(), wname);
     write_c("%s  %s%sresize", indent(), wname, arrow_or_dot);
  } else {
      write_c("new %s", t);
  }
  if (is_window()) {
    ...
    write_c("(%d, %d, %d, %d", int(o->x()), int(o->y()), int(o->w()), int(o->h()));
  }
  if (label() && *label()) {
    if(no_new) write_c(");\n%s  %s%slabel(", indent(), wname, arrow_or_dot);
    else write_c(", ");
    switch (i18n_type) {
    case 0 : /* None */
        write_cstring(label());
...
void Fl_Widget_Type::write_color(const char* field, Fl_Color color) {
  ...
  CHECK_INNER_STRUCT(buf, wname, name());
  const char *var = is_class() ? "this" : name() ? wname : "o";
  if (color_name) {
    write_c("%s%s%s%s(%s);\n", indent(), var, arrow_or_dot, field, color_name);
  } else {
    write_c("%s%s%s%s((Fl_Color)%d);\n", indent(), var, arrow_or_dot, field, color);
  }
}
...
void Fl_Widget_Type::write_widget_code() {
  ...
  if (shortcut) {
    if (use_FL_COMMAND && (shortcut & (FL_CTRL|FL_META))) {
      ...
      write_c("%s%s%sshortcut(0x%x);\n", indent(), var, arrow_or_dot, shortcut);
    }
  }
  ...
  if (is_valuator()) {
    ...
    if (v->minimum()!=f->minimum())
      write_c("%s%s%sminimum(%g);\n", indent(), var, arrow_or_dot, v->minimum());
    if (v->maximum()!=f->maximum())
      write_c("%s%s%smaximum(%g);\n", indent(), var, arrow_or_dot, v->maximum());
    if (v->step()!=f->step())
...
void Fl_Widget_Type::write_extra_code_num(int n){
    char *pc, *ec = (char*)extra_code(n);
    if (ec && !isdeclare(ec)){
      if(ec[0] == '='){ //do macro substitutions
          char buf[256];
          const char *nm;
          CHECK_INNER_STRUCT(buf, nm, name());
          nm = name() ? nm : "o";
          ...
          strec = ec;
          if(name()){
            searchAndReplace(strec, "$(name)", nm);
            searchAndReplace(strec, "$(lname)", name());
            if(nd) searchAndReplace(strec, "$(dirty_name)", nd);
          }
          if(nk) searchAndReplace(strec, "$(class)", nk);
          write_c("%s%s\n", indent(), strec.c_str());
#else
#if defined(USE_LUANOT)
          if(macro_name()){
            char buf[256];
            char buf_find[12];
            const char *ptr = macro_name();
            char *buf_ptr = buf;
            size_t str_size = 0;
            int n = 0;
            pc = 0;
            while(*ptr && (str_size < (sizeof(buf)-1))){
                *buf_ptr = *ptr;
                ++str_size;
                bool isEnd = *(ptr+1) == '\0';
                if(*ptr == ' ' || isEnd){
                    if(isEnd) *(buf_ptr+1) = '\0';
                    else *buf_ptr = '\0';
                    snprintf(buf_find, sizeof(buf_find), "$%%(%d%%)", n);
                    //if(pc) free(pc);
                    pc = ec;
                    ec = Lua->gsub(ec, buf_find, buf);
                    ++n;
                    buf_ptr = buf;
                } else {
                    ++buf_ptr;
          ...
          if(name()){
            if(nd){
              ec = Lua->gsub(ec, "%$%(dirty_name%)", nd);
            } else pc = 0;
            ec = Lua->gsub(ec, "%$%(name%)", nm);
            if(pc) free(pc);
            pc = ec;
            ec = Lua->gsub(ec, "%$%(lname%)", name());
            free(pc);
            pc = ec;
          }
          if(nk) pc = Lua->gsub(ec, "%$%(class%)", nk);
#else
          if(macro_name()){
            char buf[256];
            char buf_find[12];
...
void Fl_Widget_Type::write_properties() {
  ...
  write_string("xywh {%d %d %d %d}", int(o->x()), int(o->y()), int(o->w()), int(o->h()));
  Fl_Widget* tplate = ((Fl_Widget_Type*)factory)->o;
  if (is_spinner() && ((Fl_Spinner*)o)->type() != ((Fl_Spinner*)tplate)->type()) {
    write_string("type");
