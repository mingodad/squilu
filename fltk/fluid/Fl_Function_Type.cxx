...
void Fl_Code_Type::open() {
  ...
  if(parent->is_code_block()){
	  const char *p = ((Fl_Function_Type*)parent)->rtype();
	  if(p){
		code_parent->value(p);
		code_parent->insert(" ");
	  }
	  p = parent->name();
	  code_parent->insert(p ? p : "?");
  }
...
void Fl_Decl_Type::write_properties() {
  ...
  if (static_) 
    ...
    write_string("global");
  if(has_initialization_) write_string("has_initialization");
}
...
void Fl_Decl_Type::read_property(const char *c) {
  if (!strcmp(c,"public")) {
    ...
    static_ = 0;
  } else if (!strcmp(c,"has_initialization")) {
    has_initialization_ = 0;
  } else {
    Fl_Type::read_property(c);
  }
}
...
void Fl_Decl_Type::open() {
  ...
  decl_has_initialization->value(has_initialization_ ? 1 : 0);
  const char *c = comment();
  ...
  for (;;) { // repeat as long as there are errors
	...
	has_initialization_ = decl_has_initialization->value() == 1 ? true : false;
    if (c) free((void*)c);
    break;
  }
BREAK2:
...
void Fl_Decl_Type::write_code1() {
  ...
  // find if this is a multiline declaration
  const char* is_multiline = c;
  while (*is_multiline && (*is_multiline != '\n')) is_multiline++;

  // find the first C++ style comment
  const char* e = c+strlen(c);
  const char* csc = e;
  const char* cinit = e;
  if(!*is_multiline || has_initialization_){
      csc = c;
      ...
      // find initialization
      cinit = c;
      while ((cinit < e) && (cinit[0] != '=') && (cinit[0] != '(')) cinit++;
      if(cinit[0] == '(') cinit = e; //initialization inside function definition is ok
      else while (cinit>c && cinit[-1]==' ') cinit--; //clean spaces if any
  }
...
int Fl_Block_Type::is_public() const {return public_;}

Fl_Type *Fl_Block_Type::make() {
  Fl_Type *p = Fl_Type::current;
  while (p && !p->is_decl_block()) p = p->parent;
  Fl_Block_Type *o = new Fl_Block_Type();
  o->name("//block");
  o->public_ = 0;
  o->file_name = NULL;
  o->before = strdup("#if 1");
  o->after = strdup("#endif");
  o->add(p);
  o->factory = this;
  return o;
...
void Fl_Block_Type::write_properties() {
  Fl_Type::write_properties();
  switch (public_) {
    case 1: write_string("public"); break;
    case 2: write_string("protected"); break;
...
void Fl_Block_Type::read_property(const char *c) {
  if(!strcmp(c,"public")) {
    public_ = 1;
  } else if(!strcmp(c,"protected")) {
    public_ = 2;
  } else  if (!strcmp(c,"file_name")) {
    storestring(read_word(),file_name);
  } else  if (!strcmp(c,"before")) {
    storestring(read_word(),before);
  } else  if (!strcmp(c,"after")) {
    storestring(read_word(),after);
  } else {
    Fl_Type::read_property(c);
...
void Fl_Block_Type::open() {
  if (!block_panel) make_block_panel();
  block_name_input->static_value(name());
  block_before_input->static_value(before);
  block_after_input->static_value(after);
  block_file_name_input->static_value(file_name);
  block_public_choice->value((public_>0));
  block_panel->show();
  const char* message = 0;
  for (;;) { // repeat as long as there are errors
    if (message) fl_alert("%s", message);
    for (;;) {
      Fl_Widget* w = Fl::readqueue();
      if (w == block_panel_cancel) goto BREAK2;
      else if (w == block_panel_ok) break;
      else if (!w) Fl::wait();
    }
    const char*c = block_name_input->value();
    while (isspace(*c)) c++;
    message = c_check(c);
    if (message) continue;
    name(c);

    c = block_file_name_input->value();
    while (isspace(*c)) c++;
    storestring(c,file_name);

    c = block_before_input->value();
    while (isspace(*c)) c++;
    storestring(c,before);

    c = block_after_input->value();
    while (isspace(*c)) c++;
    storestring(c,after);

    if (public_ != block_public_choice->value()) {
      set_modflag(1);
      public_ = block_public_choice->value();
      redraw_browser();
    }
    break;
  }
...
Fl_Block_Type Fl_Block_type;

void Fl_Block_Type::write_code1() {
  const char* c = name();
  const char* cb = before ? before : "";
...
void Fl_Block_Type::write_code2() {
  const char* c = after ? after : "";
  if (public_)
    write_h("\n%s\n", c);
  write_c("\n%s\n", c);
}
