...
int Fl_Widget::take_focus() {
  ...
  if (!Fl::visible_focus() or !visible_focus()) return 0;
...
extern void delete_associations_for(Fl_Widget*); // in Fl_WidgetAssociations.cxx
...
/*! Fills the Rectangle pointed to by \a rect with the widget's
  rectangle expressed in relative to parent coordinates.
*/
void Fl_Widget::get_relative_rect( Fl_Rectangle &rect ) const {
    int ax = x();
    int ay = y();
    Fl_Widget *widget = parent_;
    if ( widget ) {
      ax -= widget->x();
      ay -= widget->y();
    }
