...
Fl_Do_at_widget_destroy_Handler Fl::do_at_widget_destroy_ = 0;
...
int Fl::getMilliSpan(int nTimeStart){
...
//           window, pushed(), grab(), modal());
  ...
  case FL_MOUSEWHEEL:
	e_dxy_timespan = getMilliSpan(e_timestamp);
	e_timestamp = getMilliCount();
	//printf("e_dxy_timespan : %d : %d : %d : %d\n", e_dxy_timespan, e_dx, e_dy, e_number);
	if(e_dxy_timespan < 50) e_dy *= (50-e_dxy_timespan)/5;
    fl_xfocus = window; // this should not happen!  But maybe it does:
...
void Fl_Widget::get_full_size(int &full_x, int &full_y, int &full_w, int &full_h) {
    int W = 0, H = 0;
    Fl_Align lbl_align = align();
    label_.measure(W, H);
    W += 5; // Add a little to the size of the label to cover overflow
    H += 5;

    //widget plus adjustments in case the label overflow
    full_x = x();
    full_y = y();
    full_w = intmax(w(), W);
    full_h = intmax(h(), H);

    if (lbl_align && !(lbl_align & FL_ALIGN_INSIDE) && !(labeltype() & FL_NO_LABEL)) {
