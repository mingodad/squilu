...
/*
 markcw: since July 2008 I have heavily modified this file and it's
 header. To make it a little easier for me to follow I made original
 code comments start in uppercase and my code comments start in lowercase.
...
struct fl_margins
{
  ...
  fl_margins(int default_margin=10) {
...
//  puts("fl_margins::clear()");
    ...
    depth_ = 0;
    return margins_[0] = default_margin_; // Default indent
  }
...
//  (depth_ > 0) ? margins_[depth_ - 1] : 4);
    ...
    if (depth_ > 0) {
      ...
      return default_margin_; // Default indent
  }
...
/* matt:
 ...
 For more info on binary compatibility (BC) see:
              ...
              ww - Fl::scrollbar_size(), Fl::scrollbar_size())
{
  ...
  mouse_y = 0;
  current_pos = 0;
  current_view = 0; // rem'd NULL
  hv_selection_color = 0;
  hv_selection_text_color = 0;

  default_margin_ = 10; //internall padding
  tr_td_adjust_ = 0; //when printint with pdfs
                        ...
                        int ww) // I - Right margin
{
  ...
  block->border_color = b.border_color; // Border color
...
Fl_Help_View::build_faces()
{
  char buf[100],
    namebuf[100],
    tbufs[100][100]; // buffers
  const char *namep; // pointer
  char *bufp,
    *namebufp; // r+w pointer
  int ti = 0,
    tj = 0,
    tk = 0, // temp loop vars
    tnum = 0,
    lnum = 0,
    fnum = 0, // temp/last/font counters
    tfi[100],
    tf[10],
    cbit = 0,
    fbit = 0,
    tbit = 0, // bits
    nfonts = 0,
    nfaces = 0, // number of fonts and faces
    temp = 0; // lengths
...
void Fl_Help_View::draw()
{
  ...
  int ti = 0, // temp loop var
    ...
    yy = 0,
    ww = 0,
    hh = 0, // Current positions and sizes
    ...
    ty = 0,
    tw = 0,
    th = 0, // Table cell positions and sizes
    linew = 0, // current line width
    imgw = 0, // Image width
    imgh = 0, // Image height
    brflag = 0; // br flag
  unsigned char  font = 0; // Current font
  ...
  ss = scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size();
        ...
        if (*ptr == '<')
        {
          ...
          *sp = '\0'; // Nul-terminate
          ...
          { // rem'd new line, added hr shadow
            ...
            if(get_attr ( attrptr, "COLOR", myattr, sizeof ( myattr ) ))
            {
                fl_color(get_color( myattr, FL_BLACK ));
                isStandard = 0;
            } else fl_color(FL_BLACK);

            if(get_attr ( attrptr, "DASH", myattr_dash, sizeof ( myattr_dash ) ))
            {
            ...
            if(get_attr ( attrptr, "HEIGHT", myattr, sizeof ( myattr ) ))
            {
                height = atoi ( myattr );
            ...
            if(get_attr ( attrptr, "WIDTH", myattr, sizeof ( myattr ) )){
            ...
            switch ( get_align ( attrptr, LEFT ) )
            {
            case CENTER :
                half1 = block->w / 2;
                half2 = width / 2;
                lx1 = block->x + tx + half1 - half2;
                lx2 = block->w + tx - half1 + half2;
                isStandard = 0;
                break;

            case RIGHT :
                lx1 = block->w + tx - width;
                lx2 = block->w + tx;
                isStandard = 0;
                break;
                   ...
                   btag == CMD('T','H',0,0)) // 'TD' 'TH'
          {
            ...
            tx = block->x - 4 - leftline_; //-4 -> left padding
            ty = block->y - topline_ - fsize - 3; //-3 -> top padding
            tw = block->w - block->x + (tr_td_adjust_ ? 6 : 7); // +7 -> right size
            th = block->h + fsize - (tr_td_adjust_ ? 6 : 5); // -5 -> bottom size
...
void Fl_Help_View::follow_link(Fl_Help_Link *lp) // I - Link pointer
{
  char target[32], // Current target
    ...
    *tptr,
    *sptr,
    *dirp; // Pointer into temporary filename
  ...
  if( link_ ){ //call link function to give it a chance to cancel it
    vlink_name = (*link_)(this, namep, lp->name);
  ...
  if (namep[0] && strcmp(namep, filename_)) // Link not same as filename_
  {
    ...
    { // dir is absolute, lp is relative
      if (namep[0] == '/') { // lp is absolute
        ...
        if ((tptr = strrchr(strchr(temp, ':') + 3, '/')) != 0) // get filename
          strlcpy(tptr, namep, sizeof(temp)-(tptr - temp)); // remove filename
        else
    ...
    { // Relative path
      if (directory_[0]) // Add filename
        ...
        dirp = getcwd(dir, sizeof(dir)); // getcwd can fail and return wrong dir, bug?
        snprintf(temp, sizeof(temp), "%s/%s", dir, namep); // No end '/' char from getcwd
      }
    }
    else // Use lp
...
void Fl_Help_View::format()
{
  ...
  int ti = 0,
    ...
    qch = 0, // Quote char
    dx = 0,
    dy = 0, // Boxtype position offsets
    dw = 0,
    dh = 0, // Boxtype sizes
    ss = 0, // Scrollbar size
    temph = 0,
    tempw = 0, // Temp scrollbar sizes
    tempx = 0,
    tempy = 0, // temp positions
    linew = 0, // current line width
    imgw = 0, // Image width
    imgh = 0, // Image height
  ...
  unsigned char thsize,
    tfsize; // font sizes
  fl_margins margins(default_margin_); // Left margin stack
  Fl_Help_Block *block, // Current block
  ...
  Fl_Color tclr,
    rclr, // Table/row background color
...
// todo: dd and dt need their own section, similar to li
        ...
        else if (b.tag == CMD('H','R',0,0)) // 'HR'
        {
          ...
          b.x = block->x;
          if (liflag) { b.x = margins.pop(); liflag = 0; } // pop li
          if(get_attr ( attrptr, "VHEIGHT", attr, sizeof ( attr ) ))
          {
            block->h += atoi ( attr );;
          } else {
            block->h += b.h;
        ...
        else if (b.tag == CMD('L','I','N',0)) // 'LINK'
        {
          ...
          if (tcss[0] == '/' || tcss[1] == ':') // absolute
            ...
            if (d->ispath) { // path is used
              ...
              if (tchar[strlen(tchar)-1] != '/')
                strlcat(tchar, "/", sizeof(tchar)); // add '/' to dir
        ...
        else if (b.tag == CMD('T','A','B',0)) // 'TABLE'
        {
          ...
          b.border = 0; // Reset
          b.border_color = FL_BLACK; // Reset
          if (get_attr(attrptr, "BORDER", attr, sizeof(attr)))
            b.border = atoi(attr);
          if (get_attr(attrptr, "BORDERCOLOR", attr, sizeof(attr)))
            b.border_color = get_color(attr, FL_BLACK);
...
//  b.x, table_width, hsize_);
          ...
          switch (get_align(attrptr, talign)) {
            ...
            case RIGHT :
              table_offset = hsize_ - table_width - fontsize_ - default_margin_; break;
          }
                           ...
                           int rc) // I - recursive counter
{
  ...
  hwidth = w() - (scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size()) - Fl::box_dw(bt);
                        ...
                        int ih) // Image height
{
  ...
  char dir[1024], // Current directory
    temp[1024], // Temporary filename
    *tptr,
    *dirp; // Pointer into temporary name
  ...
  { // dir has ':' char and np doesn't
    if (np[0] == '/') { // Has sub-path
      strlcpy(temp, directory_, sizeof(temp));
      if ((tptr = strrchr(strchr(temp, ':') + 3, '/')) != 0) // get filename
        strlcpy(tptr, np, sizeof(temp)-(tptr - temp)); // remove filename
      else
  ...
  else if (np[0] != '/' && !strchr(np, ':')) // rem'd == 0
  {
    if (directory_[0])
      ...
      dirp = getcwd(dir, sizeof(dir)); // getcwd can fail and return wrong dir, bug?
      snprintf(temp, sizeof(temp), "%s/%s", dir, np); // No end '/' char from getcwd
    }
...
Fl_Help_View::get_length(const char *lp) // I - Length pointer
{
  ...
  if (lp[strlen(lp) - 1] == '%') { // Calc percent
    ...
    val = val * (hsize_ - (scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size())) / 100; // val from hsize_
  }
...
Fl_Help_View::handle(int event) // I - Event to handle
{
  ...
  switch (event)
  {
    ...
    case FL_MOVE: // Mouse pointer was moved with no button pushed
      ...
      if (hscrollbar_.visible()) hh -= (scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size());
...
Fl_Help_View::load(const char *fp) // I - File to load, may have target
{
  ...
  char error[1024], // Error buffer
    newname[1024]; // New filename buffer
  ...
  if (link_)
    namep = (*link_)(this, newname, 0); // Link transform
  else
...
//printf(" load namep=(%s) d->nstyle=%d\n",namep,d->nstyle);
  ...
  if (!d->ispath) { // path not used
    ...
    strlcpy(directory_, newname, sizeof(directory_)); // dir only
  }
                          ...
                          int hh) // I - New height
{
  ...
  int scrollsize = scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size();
  scrollbar_.resize(x() + w() - scrollsize - Fl::box_dw(bt) + Fl::box_dx(bt),
                    y() + Fl::box_dy(bt), scrollsize, h() - scrollsize - Fl::box_dh(bt));
  hscrollbar_.resize(x() + Fl::box_dx(bt),
                     y() + h() - scrollsize - Fl::box_dh(bt) + Fl::box_dy(bt),
                     w() - scrollsize - Fl::box_dw(bt), scrollsize);
           ...
           int fc) // I - true to return what font char uses, opt
{
    ...
    static struct
    {
        ...
        cdata, // character reference data
        code, // ANSI/Unicode char code
        mac, // Mac Roman char code
        sym; // what font does char use? 0 = current, 1 = Symbol
    }
    *nameptr, // pointer into names array
    names[] =   // quoting names
    {
    ...
    int ifirst, ilast, temp;
    ilast = ( int ) ( sizeof ( names ) / sizeof ( names[0] ) ) - 1; // array size / element size
    ...
    while ( ifirst <= ilast )
    {
        ...
        {
            // found it. return
            if(fc) return nameptr->sym;
            return ENC(nameptr->code, nameptr->mac);
        }
    }
