...
HPDF_Page_Low_Level_Write  (HPDF_Page  page, const char *str)
{
    HPDF_STATUS ret;
    HPDF_PageAttr attr;

    HPDF_PTRACE ((" HPDF_Page_Low_Level_Write\n"));

    attr = (HPDF_PageAttr)page->attr;

    if ((ret = HPDF_Stream_WriteStr (attr->stream, str)) != HPDF_OK)
...
/* W n */
HPDF_EXPORT(HPDF_STATUS)
HPDF_Page_Clip_No_Border  (HPDF_Page  page)
{
    HPDF_STATUS ret = HPDF_Page_CheckState (page, HPDF_GMODE_PATH_OBJECT);
    HPDF_PageAttr attr;
    ...
    if ((ret = HPDF_Stream_WriteStr (attr->stream, "W n\012")) != HPDF_OK)
        return HPDF_CheckError (page->error);
